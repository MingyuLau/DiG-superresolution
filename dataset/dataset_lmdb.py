...
class ImageLmdb(Dataset):
  ...
  def __getitem__(self, index):
    ...
    mosaic_key = b'mosaic-%d' % index
    
    imgbuf_mosaic = self.txn.get(mosaic_key)
    buf2 = six.BytesIO()
    buf2.write(imgbuf_mosaic)
    buf2.seek(0)
    try:
      img_mosaic = Image.open(buf2).convert('RGB')
    except IOError:
      print('Corrupted mosaic_image for %d' % index)
      return self[index + 1]
    
    # mosaic_key_return = f'mosaic-{index}' 
    # filename = mosaic_key_return + '.jpg'
    # filepath = os.path.join('/home/mrchen/cmr/mosaic/DiG', filename)
    # img_mosaic.save(filepath)
    # pdb.set_trace()
    
    if self.use_aug:
      if self.use_abi_aug:
        aug_img = self.augment_abi(img)
        aug_mosaic = self.augment_abi(img_mosaic)
      else:
        # augmentation
        aug_img = self.augmentor(np.asarray(img))
        aug_img = Image.fromarray(np.uint8(aug_img))
        aug_img = self.aug_transformer(aug_img)
      return aug_img, aug_mosaic,0
    else:
      assert self.transform is not None
      img = self.transform(img)
      img_mosaic = self.transform(img_mosaic)
      return img, img_mosaic
