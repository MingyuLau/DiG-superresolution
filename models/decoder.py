...
class SRDecoder(nn.Module):
    def __init__(self,
        ...
        # self.attentions = nn.ModuleList([nn.MultiheadAttention(d_model, num_heads, dropout) for _ in range(3)])
        self.attentions = nn.ModuleList([nn.TransformerEncoderLayer(d_model, num_heads, dropout=dropout, batch_first=True) for _ in range(3)])
        
        self.linear = nn.Linear(d_model, 48)
        # self.upsample = UpsampleBLock(3,4)
        # self.mlp = nn.Linear(384, 3)
    ...
    def forward(self, x):
        ...
        Returns:
            Tensor: The raw logit tensor. Shape :math:`(N, T, C)`.
        """
        # pdb.set_trace()        # [b,256,384]  (b, 256, 128*3) (128*3, 256)   (3, 128, 256)    C * r^2, h, w       ( 3, 32, 128)
        #pdb.set_trace()
        for attention in self.attentions:
            x = attention(x)
        
        x = self.linear(x)  # [32, 256, 48]
