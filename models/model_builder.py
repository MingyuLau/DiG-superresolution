...
class RecModel(nn.Module):
  ...
  def forward(self, x):
    ...
    dec_in = self.linear_norm(enc_x)    # [32, 256, 512]
    ...
    # pdb.set_trace()  # dec_output [32, 25, 97]   dec_attn_maps[32, 25, 256] 
    # feat distillation
...
class SuperModel(nn.Module):
  ...
  def forward(self, x):
    # pdb.set_trace()
    x, x_mosaic, a = x       # x[32,3,32,128]   tgt[32,25]   tgt_lens[32]
    
    enc_x = self.encoder(x_mosaic)     # 走这一步   [32,256,384]


    dec_in = self.linear_norm(enc_x)    # [32, 256, 512]
    
    dec_output = self.decoder(dec_in)
    sr = dec_output                     # [32, 256, 384]

    mosaic_cpu = x_mosaic.cpu()
    for i, img in enumerate(mosaic_cpu):
    # 将图像转换为PIL图像
      img = np.transpose(img, (1, 2, 0))  # 将通道维度调整到最后
      img = np.uint8(np.clip(img, 0, 1) * 255)  # 将像素值转换为0-255的整数
      img = Image.fromarray(img)

      # 保存图像到文件
      filename = f'{i+1}.png'
      filepath = os.path.join('/home/mrchen/cmr/mosaic/DiG/no_pretrain/mosaic', filename)
      img.save(filepath)
    
    x_cpu = x.cpu()
    for i, img in enumerate(x_cpu):
    # 将图像转换为PIL图像
      img = np.transpose(img, (1, 2, 0))  # 将通道维度调整到最后
      img = np.uint8(np.clip(img, 0, 1) * 255)  # 将像素值转换为0-255的整数
      img = Image.fromarray(img)
      ...
      filepath = os.path.join('/home/mrchen/cmr/mosaic/DiG/no_pretrain/origin', filename)
      img.save(filepath)
    #pdb.set_trace()
    m_cpu = sr.cpu()
    for i, img in enumerate(m_cpu):
    # 将图像转换为PIL图像
      img = np.transpose(img, (1, 2, 0))  # 将通道维度调整到最后
      img = np.uint8(np.clip(img, 0, 1) * 255)  # 将像素值转换为0-255的整数
      img = Image.fromarray(img)
      ...
      filename = f'{i}.png'
      # filename = f'{a}.png'
      filepath = os.path.join('/home/mrchen/cmr/mosaic/DiG/no_pretrain/sr', filename)
      img.save(filepath)
